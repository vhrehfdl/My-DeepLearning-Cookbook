...
def data_preprocissing(train_data, valid_data, test_data, TEXT, LABEL, device):
    ...
    #
    # print(vars(train_data.text[0]))
    # print(vars(train_data.label[0]))
    # print(vars(valid_data.text[0]))
    # print(vars(test_data.text[0]))

    train_iterator = data.BucketIterator(train_data, batch_size=64, device=device, shuffle=True)
    valid_iterator = data.BucketIterator(valid_data, batch_size=64, device=device, shuffle=True)
    test_iterator = data.BucketIterator(test_data, batch_size=64, device=device, shuffle=True)

    # train_iterator, valid_iterator, test_iterator = data.BucketIterator((train_data, valid_data, test_data), batch_size=64, device=device, shuffle=True)
...
def main():
    ...
    # cifar100_model = TextSentiment(INPUT_DIM, EMBEDDING_DIM, OUTPUT_DIM)
